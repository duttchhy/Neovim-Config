...
keymap("n", "<leader>e",  function() require("oil").open() end, vim.tbl_extend("force", opts, { desc = "Open file explorer (Oil)" }))
...
keymap("n", "<leader>b0", "<cmd>BufferLineGoToBuffer -1<CR>",
...
keymap("n", "<F5>",      "<cmd>UndotreeToggle<CR><cmd>UndotreeFocus<CR>",        vim.tbl_extend("force", opts, { desc = "Toggle Undotree" }))
...
local function url_in_line_at_col(line, col)
  if not line or line == "" or not col or col < 1 then return nil end
  local from = 1
  while true do
